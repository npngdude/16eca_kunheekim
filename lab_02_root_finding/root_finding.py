...
def sequential(f, x0, delta_x=1e-6, epsilon=epsilon_global, b_verbose=False):
    """
    sequential method
    x0 로 부터 시작해서 delta_x 만큼씩증가시키면서 f(x)값이 epsilon값보다 작아지는지 관찰함
...
def bisection(f, xl, xh, epsilon=epsilon_global, b_verbose=False):
    ...
    while True:
        ...
        # 반복문이 한번 실행되었으므로 counter 를 1 증가 시킴
        counter += 1

        if b_verbose:
            # 중간 과정을 표시
            print ("xl = %8f f(xl) = %+8f xn = %+8f f(xn) = %+8f xh = %+8f f(xh) = %8f lxh-xll = %-8f" % (
...
def newton(f, df, x0, epsilon=epsilon_global, b_verbose=False):
    ...
    :param df: f(x) 의 미분
    :param x0: x의 초기값
    :param epsion: 오차 허용 한도
    :param b_verbose: 추가 정보 표시. 정해 주지 않으면 False
    :return: lf(x)l < epsilon 인 x
    """
    # xi 를 (부동소숫점) 실수로 초기화
    xi = float(x0)
    ...
    while True:
        ...
        # 반복문이 한번 실행 되었으므로 counter 를 1 증가시킴
        counter += 1
